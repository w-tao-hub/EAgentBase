...
from app.infra.tools.task_get_tool import TaskGetTool  # 导入任务获取工具
...
class Container:
    ...
    def _build_tooling_bundle(cls, settings: Settings, infra: _InfraBundle) -> _ToolingBundle:
        ...
        register_base_tool(TaskCreateTool(task_service))  # 注册任务创建工具。
        register_base_tool(TaskGetTool(task_service))  # 注册任务获取工具。
        register_base_tool(TaskUpdateTool(task_service))  # 注册任务更新工具。
        register_base_tool(TaskListTool(task_service))  # 注册任务列表工具。
    ...
    def _build_profiles(
        ...
        task_tool = TaskTool(child_runner)  # 实例化 Task 派发工具。
